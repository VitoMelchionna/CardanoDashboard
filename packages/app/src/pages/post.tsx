...
export default function PostPage() {
	...
	const [lastResponse, setLastResponse] = useState<TweetResponse | null>(
		null
	);
		...
		try {
			...
			if (response.ok && data.success) {
				toast({
					...
					description:
						data.message ||
						"Daily Cardano metrics tweet has been posted.",
					status: "success",
					duration: 5000,
					isClosable: true,
				});
		...
		<Container maxW="container.md" py={8}>
			<VStack spacing={8} align="stretch">
				...
				<Box borderWidth="1px" borderRadius="lg" p={6}>
					...
					<VStack spacing={4}>
						<Text>
							Click the button below to manually trigger the daily
							Cardano metrics tweet. This will fetch fresh data
					...
					<VStack spacing={3} align="stretch">
						<Box>
							...
							<VStack spacing={1} align="stretch" pl={4}>
								...
								<Text fontSize="sm">
									• Fetches fresh Cardano metrics from
									Blockfrost
								</Text>
								<Text fontSize="sm">
									• Formats the data into a tweet
								</Text>
								<Text fontSize="sm">
						...
						<Box>
							...
							<VStack spacing={1} align="stretch" pl={4}>
								<Text fontSize="sm">
									• <code>/api/test/daily-tweet</code> -
									Manual trigger
								</Text>
								<Text fontSize="sm">
									• <code>/api/cron/daily-tweet</code> - Cron
									job endpoint
								</Text>
								<Text fontSize="sm">
									• <code>/api/status</code> - System status
								</Text>
							</VStack>
						</Box>
					</VStack>
